...
/**
 * Creates the TOP panel. The application contains 3 panels (top, center,
 * bottom).
 * 
 * @author Daniel Cioi <dan@dancioi.net>
 * @version $Revision: 94 $ Last modified: $Date: 2013-03-30 18:07:34 +0100
 *          (Sat, 30 Mar 2013) $, by: $Author: dan.cioi $
 */
public class TopPanel extends AbsolutePanel {
	private JcsPhotoGalleryMobileView view;
	private MenuBar galleryTags;
	private ArrayList<Integer> selectedTags;

	public TopPanel(JcsPhotoGalleryMobileView view) {
		this.view = view;
		initialize();
	}
	...
	private void selectedList(int selected) {
		if (selected == 0) { // if "All Albums" is selected, then previous
								// categories are removed, and don't add it to
								// selectedTags
			selectedTags.clear();
			selectedTags.add((Integer) 0); // 0 is the id for All Albums
		} else if (selected == 1) { // if "Recent Albums" is selected, then
									// previous categories are removed, and
									// don't add it to selectedTags
			selectedTags.clear();
			selectedTags.add((Integer) 1); // 1 is the id for Recent Albums
		}

		else {
			if (selectedTags.contains(0)) { // if "All Albums" already in
											// selected tags, then remove it
				selectedTags.remove((Integer) 0);
			}
			if (selectedTags.contains(1)) { // if "Recent Albums" already in
											// selected tags, then remove it
				selectedTags.remove((Integer) 1);
			}

			if (selectedTags.contains(selected)) { // if already in selected
													// tags, then remove it
	...
	/**
	 * Adds the gallery owner home link web page. the owner is read from
	 * albums.xml file.
	 * 
	 * @param name
	...
	public void setHomePage(String name, String nameHomePage) {
		String homeLink = "<div> <a href=\"" + nameHomePage + "\">" + name
				+ "'s Web Page</a> </div>";
