...
public class SortAlbums {
	...
	public void sortAlbums(String[] albumCat1, String[] albumCat2) {
		...
		sort1(albumCat1);
		sort2(albumCat2);
		// TODO pg.addSortedCategories(getResult());
	}
		...
		for (int i = 0; i < albumCat1.length; i++) {
			if (selected == 1)
				...
				if (item1.get(selected - 3).getAlbumIndex(b) == i) {
					albumVisible[i] = true;
					if (item1.get(selected - 3).albumIndex.size() - 1 > b)
						b++;
				} else
					albumVisible[i] = false;
			} else if (selected >= 3 + item1.size() + 1) {
				if (item2.get(selected - 3 - item1.size() - 1).getAlbumIndex(c) == i) {
					albumVisible[i] = true;
					if (item2.get(selected - 3 - item1.size() - 1).albumIndex.size() - 1 > c)
						c++;
				} else
					albumVisible[i] = false;
			} else {
