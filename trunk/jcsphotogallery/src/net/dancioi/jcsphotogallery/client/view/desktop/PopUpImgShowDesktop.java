 ...
 */
public class PopUpImgShowDesktop extends PopUpImgShow {
	...
	public PopUpImgShowDesktop(int imgStart, String imgPath,
	...
	/*
	 * Add click handler on the shown image. User click on image and jump to
	 * next/previous image.
	 */
	private ClickHandler getImageClickHandler() {
		ClickHandler clickHandler = new ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				if (event.getClientX() > getBrowserWindowWidth() / 2) {
					// if the user clicks on image-right side, then go to next
					// image. Left side for previous image.
					nextImg();
				} else {
					previousImg();
				}
	...
	private void attachMouseHandlers(final AbsolutePanel controllPanel) {
		...
		this.addDomHandler(new MouseMoveHandler() {
			...
			public void onMouseMove(MouseMoveEvent event) {
				// take as movement event just if the mouse is moved with +-5
				// pixels than previous position
				if ((event.getClientX() < currentMousePositionX - ingnore || event
						.getClientX() > currentMousePositionX + ingnore)
						|| (event.getClientY() < currentMousePositionY
								- ingnore || event.getClientY() > currentMousePositionY
								+ ingnore)) {
					triggerMovement();
				}
	...
	private void computeMaximSquareSize() {
		// TODO redesign this method, as the pictures can have different formats
