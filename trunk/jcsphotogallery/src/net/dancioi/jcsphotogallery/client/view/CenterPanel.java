...
public class CenterPanel extends Grid implements GalleryAction, ReadXMLCallback {
	...
	private void showPageNr(int page, int pages) {
		if (pages == 0)
			pg.bottomPanel.setPageNr("");
		else
			pg.bottomPanel.setPageNr("page " + page + "/" + pages);
	}
	...
	public void albumPhotosCallback(AlbumBean albumPhotos) {
		...
		attachAlbumPhotos(albumPhotos.getAlbumPath(), albumPhotos.getPictures());
	}
